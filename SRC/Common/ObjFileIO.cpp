...
namespace UTILITY {
  ...
  bool load(const string fname,Objmesh &mesh){
	...
	const string err = LoadObj(shapes,fname.c_str(),dir.c_str());
	if(err.size()>0){
	  ERROR_LOG("failed to load obj mesh from: "<<fname<<",error is: "<<err);
	  return false;
	}

	WARN_LOG_COND("only support one shape with group.",(shapes.size()==1));
	assert_gt(shapes.size(),0);
	mesh.setVerts(shapes[0].mesh.positions);
	mesh.setFaces(shapes[0].mesh.indices);
	mesh.setVertNormals(shapes[0].mesh.normals);
	mesh.setMtl(shapes[0].material);
	return true;
  }
};
